...
class ReviewsController {
  ...
  //create review
  Future createReview(
      String placeID, String userID, Map<String, dynamic> data) async {
    ...
    if (!exists) {
      ...
      await _firestore
          .collection("place")
          .doc(placeID)
          .set({'average_rating': 0.0, 'total_num_ratings': 0});
    }

    await _firestore
        .collection("place/$placeID/reviews")
        .doc(userID)
        .set(data); //add user review
  ...
  Future updateReview(
    ...
    await _firestore
        .collection("place/$placeID/reviews")
        .doc(userID)
        .update(data); //update user review
    ...
    double averageRating = (ratingsSum - prevRating + data['rating']) /
        totalNumRatings; //minus previous rating, add new rating
    await _firestore.collection("place").doc(placeID).set({
      'average_rating': averageRating,
      'total_num_ratings': totalNumRatings
    });
  ...
  Future<List<Review>> returnAllReviews(String placeID) async {
    ...
    await _firestore
      ...
      querySnapshot.docs.forEach((user) async {
        Review temp = Review(
            placeID, user['userID'], user['review_text'], user['rating']);
